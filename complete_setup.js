...
function createTables() {
    ...
    db.query(createLoanDataTable, (err) => {
        ...
        // After tables are created, import data
        setTimeout(() => {
...
function importGnDivisionsData() {
    console.log('Importing GN divisions data...');
    ...
    fs.readFile(gnListPath, 'utf8', (err, data) => {
        ...
        if (totalLines === 0) {
            console.log('No GN divisions to import');
            importUsernames();
            return;
        }
        ...
        dataLines.forEach((line, index) => {
            ...
            if (trimmedLine) {
                const [division, gnDivision] = trimmedLine.split(',').map(field => field.trim().replace(/^"|"$/g, ''));
                if (division && gnDivision) {
                    const query = 'INSERT IGNORE INTO gn_divisions (division, gn_division) VALUES (?, ?)';
                    db.query(query, [division, gnDivision], (err) => {
                        ...
                        if (insertCount + (dataLines.length - insertCount) === dataLines.length) {
                            console.log(`Inserted ${insertCount} GN divisions`);
                            // Import usernames after GN divisions
                            setTimeout(() => {
                                importUsernames();
                            }, 1000);
                        }
                    ...
                    // Check completion even for invalid lines
                ...
                // Check completion for empty lines
                if (index === dataLines.length - 1) {
                    console.log(`Inserted ${insertCount} GN divisions`);
                    setTimeout(() => {
                        importUsernames();
    ...
    fs.readFile(usernamesPath, 'utf8', (err, data) => {
        ...
        users.forEach(user => {
            bcrypt.hash(user.password, parseInt(process.env.BCRYPT_SALT_ROUNDS) || 10, (err, hash) => {
                ...
                const query = 'INSERT IGNORE INTO users (username, password, bank_id, bank_name, division) VALUES (?, ?, ?, ?, ?)';
        ...
        function checkCompletion() {
            if (processedCount === users.length) {
                console.log(`Inserted ${insertCount} new users from CSV file (${users.length} total processed)`);
                finishSetup();
            }
        }
    });
...
function finishSetup() {
    ...
    console.log('- GN divisions imported');
    console.log('- Usernames and passwords imported');

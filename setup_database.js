...
function createTables() {
    ...
    db.query(createGnDivisionsTable, (err) => {
        if (err) {
            console.error('Error creating gn_divisions table:', err);
            return;
        }
        console.log('GN Divisions table created successfully');
        // Import GN divisions data
        importGnDivisionsData();
    });
    
    db.query(createLoanDataTable, (err) => {
        if (err) {
            console.error('Error creating loan_data table:', err);
            return;
        }
        console.log('Loan data table created successfully');
    });
}

function importGnDivisionsData() {
    // Read GN List CSV file
    const gnListPath = path.join(__dirname, 'GN List (1).csv');
    
    fs.readFile(gnListPath, 'utf8', (err, data) => {
        if (err) {
            console.error('Error reading GN List CSV file:', err);
            return;
        }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (line) {
                ...
                if (division && gnDivision) {
                    const query = 'INSERT INTO gn_divisions (division, gn_division) VALUES (?, ?)';
                    db.query(query, [division, gnDivision], (err) => {
                        if (err) {
                            console.error('Error inserting GN division:', err);
                        } else {
                            insertCount++;
                        }
                        // Check if we've processed all lines
                        if (insertCount + (totalLines - insertCount) === totalLines) {
                            console.log(`Inserted ${insertCount} GN divisions`);
                            // Import sample users after GN divisions are done
                            importSampleUsers();
                        }
                    });
                }
        ...
        console.log(`Processing ${totalLines} GN divisions`);
    });
}
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (line) {
                ...
                if (bankName && username && password) {
                    // Generate bank_id based on username
...
function insertSampleLoanData() {
    const sampleLoans = [
        {
            bank_id: 'BANK001',
            user_id: 1,
            beneficiary_name: 'John Perera',
            beneficiary_nic: '851234567V',
            beneficiary_address: '123 Main St, Gampaha',
            beneficiary_gn_division: 'Ganemulla',
        ...
        },
        {
            bank_id: 'BANK001',
            user_id: 1,
            beneficiary_name: 'Mary Silva',
            beneficiary_nic: '882345678V',
            beneficiary_address: '456 Oak Ave, Gampaha',
            beneficiary_gn_division: 'Yakkala',
            beneficiary_category: 'transitional',
            interest_rate: 9.00,
            loan_type: 'empowerment',
            project: 'trade',
    ...
    sampleLoans.forEach(loan => {
        const query = `
            INSERT INTO loan_data (
                bank_id, user_id, beneficiary_name, beneficiary_nic, beneficiary_address,
                beneficiary_gn_division, beneficiary_category, interest_rate, loan_type,
                project, loan_amount, loan_issued_date
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        
        const values = [
            loan.bank_id, loan.user_id, loan.beneficiary_name, loan.beneficiary_nic, loan.beneficiary_address,
            loan.beneficiary_gn_division, loan.beneficiary_category, loan.interest_rate, loan.loan_type,
            loan.project, loan.loan_amount, loan.loan_issued_date
        ];
        ...
        db.query(query, values, (err) => {
            if (err) {
                ...
                console.log(`Loan for ${loan.beneficiary_name} inserted successfully`);
                insertCount++;
                // Check if we've inserted all loans
                if (insertCount === sampleLoans.length) {
                    console.log(`Inserted ${insertCount} sample loans`);
    ...
    // If no sample loans, close connection immediately
